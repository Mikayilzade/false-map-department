...
const STABILITY_STEP := "fmd_stability_step"
const PAUSE := "fmd_pause"
...
const HISTORY_PREVIOUS := "fmd_history_previous"
const HISTORY_NEXT := "fmd_history_next"
...
static func ensure_registered() -> void:
	...
	_bind_key_once(SELECT, KEY_SPACE)
	...
	_bind_key_once(STABILITY_STEP, KEY_PERIOD)
	_bind_key_once(PAUSE, KEY_P)
	...
	_bind_key_once(SURFACE_TOGGLE, KEY_V)
	_bind_key_once(TOOL_PREVIOUS, KEY_Q)
	_bind_key_once(TOOL_NEXT, KEY_E)
	_bind_key_once(LAYER_PREVIOUS, KEY_Q, true)
	_bind_key_once(LAYER_NEXT, KEY_E, true)
	_bind_key_once(HISTORY_PREVIOUS, KEY_BRACKETLEFT)
	_bind_key_once(HISTORY_NEXT, KEY_BRACKETRIGHT)
	...
	_bind_joy_button_once(PAUSE, JOY_BUTTON_START)
	...
	_bind_joy_button_once(REGION_PREVIOUS, JOY_BUTTON_LEFT_SHOULDER)
	_bind_joy_button_once(REGION_NEXT, JOY_BUTTON_RIGHT_SHOULDER)
	...
	_bind_joy_button_once(CORRESPONDENCE, JOY_BUTTON_LEFT_STICK)
	_bind_joy_button_once(NEXT_AFFECTED, JOY_BUTTON_RIGHT_STICK)
	_bind_joy_axis_once(HISTORY_PREVIOUS, JOY_AXIS_TRIGGER_LEFT, 1.0)
	_bind_joy_axis_once(HISTORY_NEXT, JOY_AXIS_TRIGGER_RIGHT, 1.0)
...
static func replace_bindings(action: StringName, events: Array[InputEvent]) -> bool:
