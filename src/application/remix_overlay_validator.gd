...
const SAFETY_FLAGS := [
	"no_new_agent_scripts",
	"no_new_graph_topology",
	"no_new_linked_authority",
	"no_new_primitive_families",
]
...
func _validate_inputs(remix_id: String, changed: Dictionary, source: Dictionary, issues: Array) -> void:
	...
	for raw_jurisdiction in _array(source.get("jurisdictions", [])):
		jurisdictions[str(_dictionary(raw_jurisdiction).get("jurisdiction_id", ""))] = true
	...
	for raw_token in _array(source.get("semantic_label_vocabulary", [])):
		vocabulary[str(raw_token)] = true
	for raw_landmark in _array(source.get("landmarks", [])):
		for raw_token in _array(_dictionary(raw_landmark).get("allowed_semantic_labels", [])):
			vocabulary[str(raw_token)] = true
	var actual_change: bool = false

	if changed.has("initial_primitive_state"):
		var state_overrides: Dictionary = _dictionary(changed.get("initial_primitive_state", {}))
		for raw_layer_id in state_overrides.keys():
			var layer_id: String = str(raw_layer_id)
			if not layers.has(layer_id):
				_add(issues, "remix_initial_state_layer_unknown", layer_id)
				continue
			var layer: Dictionary = _dictionary(layers[layer_id])
			var initial: Dictionary = _dictionary(layer.get("initial_primitives", {}))
			var roads: Dictionary = _id_set(_array(layer.get("candidate_road_edges", [])), "edge_id")
			var waters: Dictionary = _id_set(_array(layer.get("candidate_water_edges", [])), "edge_id")
			var layer_override: Dictionary = _dictionary(state_overrides[raw_layer_id])
			for raw_field in layer_override.keys():
				var key: String = str(raw_field)
				var value: Variant = layer_override[raw_field]
				if not initial.has(key):
					_add(issues, "remix_initial_state_field_unknown", "%s:%s" % [layer_id, key])
					continue
	...
	if changed.has("agent_start_nodes"):
		var start_overrides: Dictionary = _dictionary(changed.get("agent_start_nodes", {}))
		for raw_agent_id in start_overrides.keys():
			var agent_id: String = str(raw_agent_id)
			var node_id: String = str(start_overrides[raw_agent_id])
			if not agents.has(agent_id) or not nodes.has(node_id):
				_add(issues, "remix_agent_start_unknown", "%s:%s" % [agent_id, node_id])
			elif str(_dictionary(agents[agent_id]).get("start_node_or_cell", "")) != node_id:
				actual_change = true

	if changed.has("semantic_target_assignments"):
		var target_overrides: Dictionary = _dictionary(changed.get("semantic_target_assignments", {}))
		for raw_agent_id in target_overrides.keys():
			var agent_id: String = str(raw_agent_id)
			var target: String = str(target_overrides[raw_agent_id])
			if not agents.has(agent_id) or not vocabulary.has(target):
				_add(issues, "remix_semantic_target_unknown", "%s:%s" % [agent_id, target])
			elif str(_dictionary(agents[agent_id]).get("semantic_target", "")) != target:
				actual_change = true
	...
	if changed.has("semantic_label_vocabulary"):
		...
		elif labels != _array(source.get("semantic_label_vocabulary", [])):
			actual_change = true

	if changed.has("jurisdiction_initial_ownership"):
		var ownership_overrides: Dictionary = _dictionary(changed.get("jurisdiction_initial_ownership", {}))
		for raw_cell_id in ownership_overrides.keys():
			var cell_id: String = str(raw_cell_id)
			var jurisdiction_id: String = str(ownership_overrides[raw_cell_id])
			...
			elif str(initial_jurisdiction.get(cell_id, "")) != jurisdiction_id:
				actual_change = true
	...
	if changed.has("objective_selection"):
		...
		elif _sorted_set(selected) != _sorted_keys(required_families):
			actual_change = true
...
func _id_set(items: Array, field: String) -> Dictionary:
	...
	for raw_item in items:
		out[str(_dictionary(raw_item).get(field, ""))] = true
	return out

func _all_known(values: Array, known: Dictionary) -> bool:
	for raw_value in values:
		if not known.has(str(raw_value)):
			return false
	return true
