...
func _attack_linked_authority() -> void:
	...
	var five_layers := {"layer_ids": ["L1", "L2", "L3", "L4", "L5"], "editable_fact_ids_by_layer": {}, "linked_authority_relations": []}
...
func _attack_focus_graphs() -> void:
	...
	var three_surfaces := {
		...
		"validation_metadata": {"focus_graph_by_layer": {
			"L1": _single_focus("C1"), "L2": _single_focus("C2"), "L3": _single_focus("C3"),
		}},
	}
...
func _attack_content_validation_bypasses() -> void:
	...
	var semantic := _load_json("res://content/campaign/D34.json")
	...
	var mastery := _load_json("res://content/campaign/D36.json")
	...
	var linked_budget := _load_json("res://content/campaign/D29.json")
	...
	var causal := _load_json("res://content/campaign/D39.json")
	...
	var final_case := _load_json("res://content/campaign/D40.json")
...
func _attack_production_remix_registry() -> void:
	...
	if not base.get("ok", false): return
	var production := RemixRegistryService.new().load(_dictionary(base.get("campaign_by_id", {})))
	...
	var source := _load_json("res://content/campaign/D15.json")
	var attacked := _load_json("res://content/remix/REMIX01.json")
	...
	initial["D15_L1"] = l1; changed["initial_primitive_state"] = initial; attacked["changed_inputs"] = changed
	...
	var safety_attack := _load_json("res://content/remix/REMIX02.json")
	var safety_meta: Dictionary = _dictionary(safety_attack.get("validation_metadata", {}))
	safety_meta["no_new_agent_scripts"] = false; safety_attack["validation_metadata"] = safety_meta
	_assert(_has_issue(RemixOverlayValidator.new().validate(safety_attack, _load_json("res://content/campaign/" + str(safety_attack.get("source_substrate_id", "")) + ".json")), "remix_safety_flag_missing"), "Remix agent-script escape hatch must reject")

func _rel(source: String, fact: String, target: String, projection: String) -> Dictionary:
	return {"source_layer_id": source, "source_fact_id": fact, "target_layer_id": target, "target_projection_id": projection, "projection_semantics": "fact_mirror", "direction": "one-way", "portal_ids": []}

func _neighbors() -> Dictionary:
	return {"up": "", "down": "", "left": "", "right": "", "next": "", "previous": ""}
func _single_focus(candidate_id: String) -> Dictionary:
	return {"required_focusable_candidate_ids": [candidate_id], "neighbors_by_candidate_id": {candidate_id: _neighbors()}}
...
func _has_issue(result: Dictionary, code: String) -> bool:
	for raw in _array(result.get("issues", [])):
		if str(_dictionary(raw).get("code", "")) == code: return true
	return false
...
func _find_layer(dossier: Dictionary, layer_id: String) -> Dictionary:
	for raw in _array(dossier.get("map_layers", [])):
		if str(_dictionary(raw).get("layer_id", "")) == layer_id: return _dictionary(raw)
	return {}
