...
def write_operator_guide(path: Path, source_head: str) -> None:
    path.write_text(
        "\n".join([
            ...
            "",
            "## Verify the extracted source tree",
            f"Immediately after extraction and before generating field kits, E8 packets, or T8-44 runs, execute `python3 EXTRACTED-SOURCE-VERIFY.py . <extracted-source-directory> --expected-source-head {source_head}` from the verified bundle root. This compares the complete extracted filesystem byte-for-byte against the already verified source archive and rejects missing/extra files, symlinks/special files, content changes, or executable-bit changes. The extracted directory name and any copied `SOURCE_HEAD.txt` are explicitly not trusted as source identity. Run this check before tools create generated files in the extracted tree.",
            "",
            "## Human field kit (E1-E6, E9-E11)",
            "Use only an extracted source tree that passed EXTRACTED-SOURCE-VERIFY.py for the independently retained source SHA. Prepare the v4 field kit with `scripts/phase12g_human_field_kit.py`, transport it intact, verify with `FIELD-KIT-VERIFY.py`, collect genuine observations, and finalize with `FIELD-KIT-FINALIZE.py` before repository ingest.",
            "",
            ...
            "Run Godot 4.7.1 on actual Deck-class reference hardware from the verified extracted source tree for this exact source commit. Use `tests/phase12g_reference_profile_runner.gd` with `FMD_T8_DISPOSITION=reference_run` and `FMD_T8_REFERENCE_ATTESTATION=actual_deck_class_reference`. Hosted CI and diagnostic timings are non-evidence.",
            "",
