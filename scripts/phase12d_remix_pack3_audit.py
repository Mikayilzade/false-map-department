...
def fail(m): raise SystemExit(f'PHASE12D REMIX PACK3 FAIL: {m}')
...
def nodes(src): return {n.get('node_id') for l in src.get('map_layers',[]) for n in l.get('nodes',[])}
...
def validate_inputs(remix,src):
    ...
    ls=layers(src); ag=agents(src); ns=nodes(src); fam=families(src); actual=False
    ...
    if 'agent_start_nodes' in changed:
        for aid,nid in changed['agent_start_nodes'].items():
            if aid not in ag or nid not in ns: fail(f'{rid} invalid agent start override {aid}->{nid}')
...
def main():
    ...
    prefix=[f'REMIX{i:02d}' for i in range(1,13)]
    if len(entries)!=12 or [x.get('dossier_id') for x in entries]!=prefix: fail('registry must contain exact contiguous REMIX01-REMIX12')
    transforms=set()
    expected_sources={'REMIX09':'D22','REMIX10':'D37','REMIX11':'D12','REMIX12':'D10'}
    for i in range(9,13):
        rid=f'REMIX{i:02d}'; remix=load(REM/f'{rid}.json')
        if remix.get('dossier_id')!=rid or remix.get('remix_schema_version')!=1 or remix.get('remix_pack_id')!='PACK03': fail(f'{rid} identity/schema/pack invalid')
        sid=remix.get('source_substrate_id','')
        if sid!=expected_sources[rid] or not re.fullmatch(r'D\d{2}',sid) or not (CAM/f'{sid}.json').exists(): fail(f'{rid} source substrate invalid')
    ...
    if len(transforms)<3: fail('PACK03 must contain at least three distinct reasoning transformations')
    print('Phase 12D Remix Pack 3 audit: PASS (REMIX09-REMIX12 P10-R10, bounded source overlays)')
